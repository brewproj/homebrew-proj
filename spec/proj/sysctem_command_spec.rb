...
describe Hbp::SystemCommand do
  describe "when the exit code is 0" do
    let(:command) { File.exist?('/bin/true') ? '/bin/true' : '/usr/bin/true' }
  ...
  describe "when the exit code is 1" do
    let(:command) { File.exist?('/bin/false') ? '/bin/false' : '/usr/bin/false' }

    describe "and the command must succeed" do
      it "throws an error" do
        expect {
          described_class.run!(command)
        }.to raise_error(Hbp::ProjectCommandFailedError)
      end
    end
