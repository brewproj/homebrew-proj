...
require 'hbp/utils/tty'

UPDATE_CMD = "brew update && brew upgrade brew-proj && brew cleanup && brew proj cleanup"
ISSUES_URL = "https://github.com/brewproj/homebrew-proj/issues"
...
def ohai(title, *sput)
  title = Tty.truncate(title) if $stdout.tty? && !Hbp.verbose
...
def odebug(title, *sput)
  if Hbp.respond_to?(:debug) and Hbp.debug
    width = Tty.width * 4 - 6
...
def puts_columns(items, star_items=[])
  ...
  puts Hbp::Utils.stringify_columns(items, star_items)
end
...
module Hbp::Utils
  ...
  def self.cabv(dir)
    output = ''
    count = Hbp::SystemCommand.run!('/usr/bin/find',
                                     :args => [dir, *%w[-type f -not -name .DS_Store -print0]],
                                     :print_stderr => false).stdout.count("\000")
    size = Hbp::SystemCommand.run!('/usr/bin/du',
                                    :args => ['-hs', '--', dir],
  ...
  def self.error_message_with_suggestions
    <<-EOS.undent
    #{ Tty.reset.bold }
      Most likely, this means you have an outdated version of homebrew-proj.#{
      } Please run:
