  ...
  module ClassMethods
    ...
    def appcast(*args)
      ...
      if @appcast and !args.empty?
        raise Hbp::ProjectInvalidError.new(self.token, "'appcast' stanza may only appear once")
      end
      @appcast ||= begin
        Hbp::DSL::Appcast.new(*args) unless args.empty?
      rescue StandardError => e
    ...
    def gpg(*args)
      ...
      if @gpg and !args.empty?
        raise Hbp::ProjectInvalidError.new(self.token, "'gpg' stanza may only appear once")
      end
      @gpg ||= begin
        Hbp::DSL::Gpg.new(*args) unless args.empty?
      rescue StandardError => e
        raise Hbp::ProjectInvalidError.new(self.token, e)
      end
    end
    ...
    def container(*args)
      ...
      if @container and !args.empty?
        ...
        raise Hbp::ProjectInvalidError.new(self.token, "'container' stanza may only appear once")
      end
      @container ||= begin
        Hbp::DSL::Container.new(*args) unless args.empty?
      rescue StandardError => e
        raise Hbp::ProjectInvalidError.new(self.token, e)
      end
    ...
    def version(arg=nil)
      if arg.nil?
        ...
        raise Hbp::ProjectInvalidError.new(self.token, "'version' stanza may only appear once")
      elsif !arg.is_a?(String) and !SYMBOLIC_VERSIONS.include?(arg)
        raise Hbp::ProjectInvalidError.new(self.token, "invalid 'version' value: '#{arg.inspect}'")
      end
      @version ||= arg
    end
    ...
    def sha256(arg=nil)
      if arg.nil?
        ...
        raise Hbp::ProjectInvalidError.new(self.token, "'sha256' stanza may only appear once")
      elsif !arg.is_a?(String) and !SYMBOLIC_SHA256S.include?(arg)
        raise Hbp::ProjectInvalidError.new(self.token, "invalid 'sha256' value: '#{arg.inspect}'")
      end
      @sha256 ||= arg
    end
    ...
    def tags(*args)
      ...
      if @tags and !args.empty?
        ...
        raise Hbp::ProjectInvalidError.new(self.token, "'tags' stanza may only appear once")
      end
      @tags ||= begin
        Hbp::DSL::Tags.new(*args) unless args.empty?
      rescue StandardError => e
        raise Hbp::ProjectInvalidError.new(self.token, e)
      end
    end
    ...
    def license(arg=nil)
      ...
      if @license and !arg.nil?
        raise Hbp::ProjectInvalidError.new(self.token, "'license' stanza may only appear once")
      end
      @license ||= begin
        Hbp::DSL::License.new(arg) unless arg.nil?
      rescue StandardError => e
        raise Hbp::ProjectInvalidError.new(self.token, e)
      end
    end
    ...
    def conflicts_with(*args)
      if @conflicts_with and !args.empty?
        ...
        raise Hbp::ProjectInvalidError.new(self.token, "'conflicts_with' stanza may only appear once")
      end
      return @conflicts_with if args.empty?
      @conflicts_with ||= begin
        Hbp::DSL::ConflictsWith.new(*args) unless args.empty?
      rescue StandardError => e
        raise Hbp::ProjectInvalidError.new(self.token, e)
      end
    end
    ...
    def accessibility_access(accessibility_access=nil)
      if @accessibility_access and !accessibility_access.nil?
        raise Hbp::ProjectInvalidError.new(self.token, "'accessibility_access' stanza may only appear once")
      end
    ...
    ordinary_artifact_types.each do |type|
      define_method(type) do |*args|
        if type == :stage_only and args != [true]
          raise Hbp::ProjectInvalidError.new(self.token, "'stage_only' takes a single argument: true")
        end
        artifacts[type] << args
        if artifacts.key?(:stage_only) and
          artifacts.keys.count > 1 and
          ! (artifacts.keys & Hbp::DSL::ClassMethods.activatable_artifact_types).empty?
          raise Hbp::ProjectInvalidError.new(self.token, "'stage_only' must be the only activatable artifact")
        end
      end
    end
    ...
    def installer(*args)
      ...
      begin
        artifacts[:installer] << Hbp::DSL::Installer.new(*args)
