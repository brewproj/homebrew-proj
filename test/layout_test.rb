...
describe "Repo layout" do
  ...
  describe "Projects dir" do
    it "finds some files in the Projects dir" do
      entries = Dir.entries(project_root.join('Projects'))
      entries.length.must_be :>, 0
    end

    it "only finds .rb files in the Projects dir" do
      entries = Dir.entries(project_root.join('Projects')) - IGNORE_FILES
      IGNORE_REGEXPS.each do |regexp|
        entries.reject! { |elt| elt.match(regexp) }
      end
